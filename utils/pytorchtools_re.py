...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        if self.verbose:
            print(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...') # self.val_loss_min
...
class Ontology_perturbation(object):  # ontology_perturbation_detection
    def __init__(self, subgraphs_all, ratio, device, mode = 'normal'):
        super(Ontology_perturbation, self).__init__()
        self.N_neg = None
    ...
    def perturbation(self, batch_graph):
        ...
        if self.mode == 'zero':
            ...
            while cnt > 0 :
                ...
                cnt -=1   
            # cols = list(range(self.ntype))
            # pert_idx = list(itertools.combinations(cols, cnt))
            clean_candidates = [] 
    ...
    def perturbation2(self, batch_graph):
        ...
        for pert in pert_idx:
            ...
            for col in pert:
                subgraph_pert[:,col] = col_dic[col][torch.randperm(col_dic[col].shape[0])] # 此处subgraph_pert中可能存在重复行，需要考虑去重
                pert_pattern[:,col] = 1
            # clean_candidates.append(batch_graph)
    ...
    def get_perturbation2(self, batch_graph):
        ...
        while len(perturbation[0]) < 2 * self.N_neg:
            ...
            candidates_eq_batch_graph = torch.sum(candidates_eq_batch_graph,dim=1).bool()
            candidates = candidates[~candidates_eq_batch_graph] # 去除candidate中的正样本
...
def link_prediction_by_graph(test_subgraph, adjM, embedding):   # 以下以adjM的行数和列数均为 len(nodes)
    # adjM = adjM.A
    nodes = np.unique(test_subgraph)    # test_subgraph在此处仅用于提取nodes
    ...
    # adjM = adjM.A[np.ix_(nodes,nodes)]
    ...
    neg_sample_num = len(pos_data)     # 负采样数量, 与正样例相同 （或正例数量的某个比例
...
def link_prediction_by_graph2(test_subgraph, adjM, embedding):   
    adjM = adjM.A
    nodes = np.unique(test_subgraph)    # test_subgraph for extracting nodes only
    ...
    pos_data = np.array(pos_data)
    num_neg_candidates = len(neg_candidates)
