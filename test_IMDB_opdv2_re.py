...
from utils.pytorchtools_re import EarlyStopping, FocalLoss, Ontology_perturbation, link_prediction_by_graph2
import torch
...
save_postfix = 'msgnn_imdb'
sys.stdout = Logger(r'./train_log/' + save_postfix +'.log', sys.stdout)
data_path = r'./data/IMDB_processed/graph_split'
device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

features, subgraphs,subgraphs_train_val_test, train_val_test_idx, labels, adj, type_mask = load_IMDB_data(prefix = data_path)
...
for rpt in range(repeat):
    ...
    early_stopping = EarlyStopping(patience=patience, verbose=True, save_path= r'./model/checkpoint_{}_{}.pt'.format(save_postfix, rpt))
    ...
    for epoch in range(num_epochs):
        ...
        for iteration in range(batch_g.num_iterations()):
            ...
            features_train, y_pred_node,y_pred_graph = net((batch_features, batch, perturbation))
            ...
            y_pred_node = y_pred_node.reshape(-1,ntype).reshape(-1,1)
            OPD_loss = gamma * FLloss(y_pred_node, node_label) + F.binary_cross_entropy_with_logits(y_pred_graph, label)
        ...
        with torch.no_grad():
            for iteration in range(val_g.num_iterations()):
                ...
                y_pred_node = y_pred_node.reshape(-1,ntype)[~zero_rows].reshape(-1,1)
                OPD_loss = gamma * FLloss(y_pred_node, node_label) + F.binary_cross_entropy_with_logits(y_pred_graph, label)
                ...
                auc_opd_node.append(roc_auc_score(node_label.cpu(), y_pred_node.cpu()))
                ap_opd_node.append(average_precision_score(node_label.cpu(), y_pred_node.cpu()))
    ...
    net.load_state_dict(torch.load(r'./model/checkpoint_{}_{}.pt'.format(save_postfix, rpt)))
    net.eval()
    ...
    with torch.no_grad():
        for iteration in range(test_g.num_iterations()):
            ...
            pos_out, neg_out, y_true_test = link_prediction_by_graph2(batch_adj.cpu(), adj, features_test)
            ...
            ap_lp = average_precision_score(y_true_test, y_proba_test)
