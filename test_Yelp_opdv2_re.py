...
save_postfix = 'msgnn_yelp'
sys.stdout = Logger(r'./train_log/' + save_postfix +'.log', sys.stdout)
data_path = r'./data/yelp2_processed/graph_split'
device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

features, subgraphs,subgraphs_train_val_test, train_val_test_idx, labels, adj, type_mask = load_Yelp2_data(prefix = data_path)
...
for rpt in range(repeat):
    ...
    net.load_state_dict(torch.load('./ckpt/checkpoint_{}_{}.pt'.format(save_postfix, rpt)))
    ...
    with torch.no_grad():
        for iteration in range(test_g.num_iterations()):
            ...
            precision_lp, recall_lp, _ = precision_recall_curve(y_true_test, y_proba_test_label)
