...
beta_2 = 0.7
nheads_1 = 4 # intro subgraph
nheads_2 = 4 # inter subgraph

save_postfix = 'dblp_opd_v2re_0221_addfc_t1640'
rpt = 0

data_path = r'./data/DBLP_processed/graph_split'

device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')


features, subgraphs,subgraphs_train_val_test, train_val_test_idx, labels, adj, type_mask, node_ontology, ontology_path, ontology_pairs, _, __ = load_DBLP_data2(prefix = data_path)
...
net = OHNN_v3(ontology_path, ontology_pairs, in_dims, hidden_dim, out_dim, nheads_1, dropout_rate, alpha, beta_1, device, g_readout, ohnn_agg, opd_readout)
net.to(device)
...
if __name__ == '__main__':
    node_representations = representations_generating_v2(net, subgraphs, features, type_mask, device, save_flag = True, max_size=10000, task = 'addfc_all')
    node_representations = np.load(data_path + '/representations/' + 'addfc_all' + '_node_representations.npy') # opd_v2_88
